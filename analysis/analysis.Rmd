---
title: "Bo-NO-bouba-kiki: Picture-word mapping but not sound symbolic speech-shape mapping in a language trained bonobo"
...
```{r rtmodel, fig.cap = "Estimates for condition, trial and interaction between trial and condition (with 95\\% CrI) in the model for reaction times. Dashed line shows chance level. Solid black line highlights a break in the x-axis", out.width="100%", fig.height = 3}
bm_rt%>%
...
The model reported above used default priors as implemented in the `brms` package. Here we explore the effect of different prior distributions for the intercept estimate on the results. The default prior in `brms` is `student_t(3, 0, 2.5)`. We compare this to six other priors. First, three priors that are also centered at `0` (i.e. assume no effect) and then three priors that assume a very strong effect (centered at `1` which corresponds to 73% correct responses). For every effect type we compare a weakly informative prior: `normal(0/1, 10)`, an informative prior: `normal(0/1, 5)`, and a highly informative prior: `normal(0/1, 1)`. Figure \@ref(fig:priorsens) visualizes the estimates given the different prior distributions and shows that the prior settings had little to no influence on the posterior estimate in the model. 
...
#   fixef(bm_test_weak_effect_prior)%>%as_tibble()%>%mutate(Prior = "weakly informative", Effect = "Strong effect"),
#   fixef(bm_test_medium_effect_prior)%>%as_tibble()%>%mutate(Prior = "informative", Effect = "Strong effect"),
#   fixef(bm_test_strong_effect_prior)%>%as_tibble()%>%mutate(Prior = "highly informative", Effect = "Strong effect")
# )
# 
...
```{r priorsens, fig.cap = "Estimate for intercept term (with 95\\% CrI) in the model for test trials with different prior distributions. Dashed line shows chance level (in link space). *No effect* refers to prior distributions centered at zero, *strong effect* refers to prior distributions centered at one.", out.width="100%", fig.height = 2}
...
Figure \@ref(fig:retmodel) shows that the posterior estimate for the intercept was clearly positive and the 95% CrI did not overlap with 0. Thus, Kanzi was clearly able to matcht the sounds to the correct pictures. 
...
```{r retmodel, fig.cap = "Estimate for intercept term (with 95\\% CrI) in the model for regular trials. Dashed line shows chance level (in link space).", out.width="100%", fig.height = 2}
