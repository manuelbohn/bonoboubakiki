...
```
Change in reaction time over time. Reaction times are relatively stable across time and changed in simialr ways in regular and test trials.
...
rt_data%>%
  ...
  labs(x = "Trial (continous across sessions)", y = "Reaction time")+
...
We used a Bayesian generalized linear mixed model to analyze performance. In such a model, an intercept of 0 in link space corresponds to a proportion of correct choices of 0.5. Thus, we inferred that if the 95% credible interval around the intercept estimate did not include 0, performance is reliably above chance.
...
Here we explore the effect of different prior distributions for the intercept term on the results. The default prior is `student_t(3, 0, 2.5)`. We compare this to three other priors. A weak prior `normal(0, 10)`, a medium prior `normal(0, 5)`, and a strong prior `normal(0, 1)`.
