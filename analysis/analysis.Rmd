...
ggplot(plot, aes(x = condition))+
  ...
  geom_jitter(data = plot_session, aes(x= condition, y = mean, col = condition), alpha = 0.5, width = 0.05, height = 0)+
...
```{r}
bm_shape <- brm(correct ~ test_shape + (1|SampleSound) + (1 | shape_combination),
          data = test_data,
          family = bernoulli(),
          cores = 4,
          chains = 4,
...
```{r}
fm_test <- glmer(correct ~ test_shape + (1|SampleSound) + (1 | shape_combination), 
              data = test_data, 
              family = binomial, 
              control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
