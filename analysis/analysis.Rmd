...
The model reported above used default priors as implemented in the `brms` package. Here we explore the effect of different prior distributions for the intercept estimate on the results. The default prior in `brms` is `student_t(3, 0, 2.5)`. We compare this to six other priors. First, three priors that are also centered at zero (i.e. assume no effect) and then three priors that assume a very strong effect (centered at one which corresponds to 73% correct responses). For every effect type we compare a weakly informative prior `normal(0, 10)`, an informative prior `normal(0, 5)`, and a highly informative prior `normal(0, 1)`. Figure \@ref(fig:priorsens) visualizes the estimates given the different prior distributions and shows that the prior settings had little to no influence on the posterior estimate in the model. 
